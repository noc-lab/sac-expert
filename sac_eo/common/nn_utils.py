...
def create_initializer(init_type,gain=None):
    ...
    if init_type == 'orthogonal':
        ...
        # Default TF initialization used by Haarnoja softlearning code
        init = 'glorot_uniform'
    else:
...
def create_nn(in_dim,out_dim,layers,activations,gain,init_type='orthogonal',layer_norm=False,name=None):
...
def create_embedding(embed_dim,out_dim,activation,init_type,gain):
    """Creates linear embedding with activation.
    
    Args:
        embed_dim (int): dimension of embedding
        out_dim (int): dimension of neural network output
        activation (str): activation for neural network
        init_type (str): initialization type
        gain (float): multiplicative factor for layer initialization
    
    Returns:
        Linear TensorFlow layer followed by non-linearity.
    """
    nn = tf.keras.Sequential()

    activation = create_activations([embed_dim],[activation])[0]
    initialization = create_initializer(init_type,gain)
    
    nn.add(create_layer(out_dim,initialization,activation,embed_dim))
